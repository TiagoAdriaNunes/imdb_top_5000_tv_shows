...
required_columns <- c("primaryTitle", "startYear", "rank", "averageRating", "numVotes",
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      menuItem(HTML("Top 5000 TV Shows<br>Last Update: 2024-09-23"),
  ...
  dashboardBody(
    # Apply the custom theme CSS (if any)
    tags$head(
      # You can include custom CSS or JS here
    ),
    fluidRow(
      box(
        title = "Best Genres by TV Shows",
        width = 12,
        shinycssloaders::withSpinner(
          plotlyOutput("plot_genres_by_tvshows"),
          type = 3,
          color = "#427ea6",
...
server <- function(input, output, session) {
  ...
  filter_data <- function() {
    ...
    if (length(input$director) > 0) {
      filtered <- filtered %>%
        filter(sapply(strsplit(as.character(directors), ",\\s*"),
                      function(d) any(input$director %in% d)))
    }
    
    # Filter by writers
    if (length(input$writer) > 0) {
      filtered <- filtered %>%
        filter(sapply(strsplit(as.character(writers), ",\\s*"),
                      function(w) any(input$writer %in% w)))
    }
    ...
    if (length(input$genre) > 0) {
      filtered <- filtered %>%
        filter(sapply(strsplit(as.character(genres), ",\\s*"),
                      function(g) all(input$genre %in% g)))
    }
  ...
  output$dataTable <- renderReactable({
    reactable(filteredData() %>%
                select(Title_IMDb_Link, startYear, rank, averageRating, numVotes, genres),
              columns = list(
  ...
  output$plot_genres_by_tvshows <- renderPlotly({
    plot_data <- filteredData() %>%
      separate_rows(genres, sep = ",\\s*") %>%
      group_by(genres) %>%
      summarise(tvshow_count = n()) %>%
      arrange(desc(tvshow_count)) %>%
      head(input$num_results) %>%
      mutate(genres = factor(genres, levels = rev(unique(genres))))
    
    create_plot(plot_data, ~tvshow_count, ~genres, "Number of TV Shows", "Genre")
  })
  ...
  observeEvent(input$reset, {
    updateSliderInput(session, "year",
                      value = c(min(data$startYear, na.rm = TRUE), max(data$startYear, na.rm = TRUE)))
    updateSliderInput(session, "rank",
                      value = c(min(data$rank, na.rm = TRUE), max(data$rank, na.rm = TRUE)))
